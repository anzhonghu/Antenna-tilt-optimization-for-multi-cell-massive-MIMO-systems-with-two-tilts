...
for i=1:Ni
    for q=1:length(Qs)
        ...
        sumrate(:,M/10)=sumrate(:,M/10)+sum;                    %实际值
        sumrate1(:,M/10)=sumrate1(:,M/10)+K*log2(1+P/sum1);     %极限值
        sumrate2(:,M/10)=sumrate2(:,M/10)+sum2;                 %倾角指向中心
        sumrate3(:,M/10)=sumrate3(:,M/10)+sum3;                 %倾角指向两个扇区
    end
end
sumrate(:,:)=sumrate(:,:)/Ni;
...
plot(Qss,sumrate(:,:),'ks-'),xlabel('Number of the BS antenna M'),ylabel('Sum rate[bps/Hz]');
hold on;
plot(Qss,sumrate1(:,:),'gd-'),xlabel('Number of the BS antenna M'),ylabel('Sum rate[bps/Hz]');
plot(Qss,sumrate2(:,:),'bx-'),xlabel('Number of the BS antenna M'),ylabel('Sum rate[bps/Hz]');
plot(Qss,sumrate3(:,:),'cp-'),xlabel('Number of the BS antenna M'),ylabel('Sum rate[bps/Hz]');
...
for q=1:length(Qs)
    ...
    sum1 = 0;
    for m = 1:N
        phi = ((m-1)/N)*(2*pi/3)-(pi/3);
        for n = 1:N
            theta3 = th1 + ((n-1)/N)*(dthe);
            d=h/sin(theta3);
            ...
            sum1 = sum1 + K*((1 /(M1*(alphatheta1^2) + M2*(alphatheta2^2)))*(3/(2*pi))...
                *((2*(h^2))/(r1^2-r2^2))*cot(theta3)*(1+(cot(theta3))^2)*((2*pi)/(3*N))*(dthe/N));
        end
    end
    R=K*log2(1+(P/sum1));
    R1(:,M/10)=R;
end
plot(Qss,R1(:,:),'-ro'),xlabel('Number of the BS antenna M'),ylabel('Sum rate[bps/Hz]');
grid on;
