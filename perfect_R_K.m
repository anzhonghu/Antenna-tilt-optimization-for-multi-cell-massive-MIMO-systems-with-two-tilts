...
for i=1:Ni
    for q=1:length(Qs)
        ...
        sumrate(:,K/5)=sumrate(:,K/5)+sum;                    %实际值
        sumrate1(:,K/5)=sumrate1(:,K/5)+K*log2(1+P/sum1);     %极限值
        sumrate2(:,K/5)=sumrate2(:,K/5)+sum2;                  %倾角指向中心
        sumrate3(:,K/5)=sumrate3(:,K/5)+sum3;
    end
end
sumrate(:,:)=sumrate(:,:)/Ni;
...
plot(Qss,sumrate2(:,:),'bx-'),xlabel('Number of the user K'),ylabel('Sum rate[bps/Hz]');
plot(Qss,sumrate3(:,:),'cp-'),xlabel('Number of the user K'),ylabel('Sum rate[bps/Hz]');
...
plot(Qss,R1(:,:),'-ro'),xlabel('Number of the user K'),ylabel('Sum rate[bps/Hz]');
grid on;
