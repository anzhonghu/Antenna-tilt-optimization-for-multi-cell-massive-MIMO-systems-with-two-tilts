...
sumrate = zeros(1,10);
sumrate1 = zeros(1,10);
sumrate_1 = zeros(1,10);
sumrate_2 = zeros(1,10);
R = zeros(1,10);
th1 = pi/2 - atan(r1/h);
th2 = pi/2 - atan(r2/h);
dthe = th2 - th1;
...
for i = 1:Ni
    ...
    for q=1:length(Qs)
        ...
        sumrate(:,M/10)=sumrate(:,M/10)+sum;
        sumrate1(:,M/10)=sumrate1(:,M/10)+sum0;
        sumrate_1(:,M/10)=sumrate_1(:,M/10)+sum_1;
        sumrate_2(:,M/10)=sumrate_2(:,M/10)+sum_2;
    end
end
sumrate(:,:)=sumrate(:,:)/Ni;
...
plot(Qss,sumrate_1(:,:),'bx-'),xlabel('Number of the BS antenna M'),ylabel('Sum rate[bps/Hz]');
plot(Qss,sumrate_2(:,:),'cp-'),xlabel('Number of the BS antenna M'),ylabel('Sum rate[bps/Hz]');
