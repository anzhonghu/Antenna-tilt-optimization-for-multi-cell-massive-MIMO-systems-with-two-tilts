...
for i = 1:Ni
    ...
    for q=1:length(Qs)
        ...
        for k = 1 : K
            %式（41）：极限值
            numer(k,1)=((M1*((alphatheta1_1(:,k))^2)+M2*((alphatheta2_1(:,k))^2))/(M1*((alphatheta1_1(:,k))^2+(alphatheta1_5(:,k))^2+(alphatheta1_8(:,k))^2)+...   %式（37）、（39）
        ...
        sumrate1(:,q)=sumrate1(:,q)+sum0;
        sumrate_1(:,q)=sumrate_1(:,q)+sum_1;
        sumrate_2(:,q)=sumrate_2(:,q)+sum_2;
        %R(:,q)=R(:,q) + K*log2(1+(1/((L-1)*(sum4/K)*(sum5/K))));
    end
end
sumrate(:,:)=sumrate(:,:)/Ni;
...
plot(Qs,sumrate_1(:,:),'bx-'),xlabel('SNR[dB]'),ylabel('Sum rate[bps/Hz]');
plot(Qs,sumrate_2(:,:),'cp-'),xlabel('SNR[dB]'),ylabel('Sum rate[bps/Hz]');
% plot(Qs,R(:,:),'ro-'),xlabel('SNR[dB]'),ylabel('Sum rate[bps/Hz]');
grid on;
